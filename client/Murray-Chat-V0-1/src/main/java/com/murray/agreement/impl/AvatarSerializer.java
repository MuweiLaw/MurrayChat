...
public class AvatarSerializer implements Serializer {
    ...
    public <T> T deserialize(Class<T> clazz, byte[] bytes) {
        ...
        //获取当时填写的文件路径
        File file = new File(ClientCache.AVATAR_FOLDER,fileName);
        ...
        try {
            ...
            String userNo = fileName.substring(0, fileName.lastIndexOf("."));
            if (userNo.equals(ClientCache.chatUserByLogIn.getChatUserNo())||userNo.equals("defaultUserAvatar")) {
                CHAT_MAIN_FRAME.headSculpture.setIcon(PictureManipulationUtil.getUserAvatar(userNo, 40, 40));
            }
        }
        }).start();
        return null;
